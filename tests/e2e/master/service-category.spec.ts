...
test.describe('Master Data - Service Category', () => {
    test.beforeEach(async ({ page }) => {
        await page.goto('/master/service-category')
        await page.waitForLoadState('domcontentloaded')
    })

    test('should display service category list table', async ({ page }) => {
        await expect(page.locator('table').first()).toBeVisible({ timeout: 15000 })
        await expect(page.locator('.page-title').first()).toContainText(/หมวดหมู่บริการ/)
    })

    test('should create a new service category', async ({ page }) => {
        const data = TEST_DATA.serviceCategory
